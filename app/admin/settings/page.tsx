  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={save} className="card space-y-6 p-6">
        ...
        <div>
          <h2 className="mb-3 font-serif text-2xl text-accent">
            Фон главного экрана
          </h2>
          <p className="mb-4 text-sm text-white/60">
            Выберите фото из галереи — они будут плавно сменяться размытым фоном
            на главном экране.
          </p>
          {photos.length === 0 ? (
                  ...
                  <button
                    ...
                    style={{
                      borderColor: selected
                        ? "var(--color-accent, #c9a84c)"
                        : "transparent",
                    }}
                    title={photo.title}
                  >
                    <img
                      src={`https://res.cloudinary.com/dt70epmum/image/upload/w_200,q_auto,f_auto/${photo.filename}`}
                      alt={photo.title}
          ...
          {heroPhotoIds.length > 0 && (
            <p className="mt-2 text-sm text-white/50">
              Выбрано: {heroPhotoIds.length} фото
            </p>
          )}
        </div>

        <button className="rounded bg-accent px-4 py-2 text-black">
          Сохранить
        </button>
      </form>
