...
5. Лайтбокс: навигация клавишами/свайп, на последнем фото клик закрывает.
6. Прямая загрузка файла — фото появляется в галерее и отображается.
7. Импорт папки — категория создаётся, фото видны на витрине, оригиналы на диске не трогались.
8. Открыть `/api/photos/image?key=...` напрямую — должно отдать байты изображения.
