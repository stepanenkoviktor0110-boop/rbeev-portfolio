...
3. Лайтбокс: навигация клавишами/свайп, на последнем фото клик закрывает.
4. Прямая загрузка файла — фото появляется в галерее и отображается.
5. Импорт папки — категория создаётся, фото видны на витрине, оригиналы на диске не трогались.
6. Открыть `/api/photos/image?key=...` напрямую — должно отдать байты изображения.
