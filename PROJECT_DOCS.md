...
Кэширование:
- `Cache-Control: public, max-age=86400` на стороне браузера.
- На сервере временные ссылки Яндекса кэшируются в памяти процесса на короткое время
  (по умолчанию ~4 минуты), чтобы не ходить в Yandex API на каждый запрос.
- Если временная ссылка успела протухнуть, прокси один раз запрашивает новую и повторяет стрим.
...
- Мобильная плашка навигации увеличена по высоте; шрифт и расстояние между ссылками увеличены для читаемости.
...
- Карточки дат сделаны шире (примерно +25% по ширине относительно предыдущей версии).
- Занятые даты выделяются более насыщенным красным оттенком (фон, замок, подпись статуса).
- Подпись месяца:
...
- Клик по видимым стрелкам (слева/справа от фото) всегда переключает кадр.
